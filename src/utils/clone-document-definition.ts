...
const cloneRecursive = <T>(value: T, ancestors: WeakMap<object, object>): T => {
	...
	if (Array.isArray(value)) {
		...
		for (const item of value) {
			clone.push(cloneRecursive(item, ancestors));
		}
	...
	for (const key of Reflect.ownKeys(value)) {
		clone[key] = cloneRecursive((value as Record<PropertyKey, unknown>)[key], ancestors);
	}
	ancestors.delete(value);
	return clone as T;
};
