...
- Removed the pass-through `LayoutBuilderContent` and `DocMeasureContainers` classes; layout and measurement composition roots now supply feature contexts directly while retaining test-facing measurement façades.
...
- Moved shorthand normalization, ordered handler construction, concrete feature preprocessing contexts, extension fallback and TOC registration out of `DocPreprocessor` into preprocessing composition; `DocPreprocessor` now retains document state, reference tracking, recursive entry points and compatible overridable façades.
- Extracted vector-insertion ownership tracking into a shared layout service, removing the table feature's dependency on the concrete element writer while retaining its existing façade.
...
- Moved built-in measurement context construction, shared image/text measurers, handler registration, style-scoped node lifecycle, margin extension, extension fallback and unknown-node errors into a composition root; `DocMeasure` now owns only shared measurement state and compatible feature façades.
...
- Moved primary/trailing layout order, initial-page decisions, built-in feature handler construction and stack/section/column context creation out of `LayoutBuilder` into a layout composition root. Moved shared table/columns row layout under the table feature and let composition own it, while keeping recursive/row compatibility façades and subclass dispatch behavior intact.
