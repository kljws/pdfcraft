...
class DocMeasure {
	...
	constructor(
		...
		this.media = new DocMeasureMedia(this.pdfDocument, this.styleStack);
	...
	measureNode(node: PreprocessedPdfNode): MeasuredPdfNode {
		...
		return this.styleStack.auto(measuredNode, () => {
			...
			if (measuredNode.section) {
				...
				const extension = this.extensions.find((candidate) =>
					candidate.test(measuredNode as ExtensionNode),
				);
				if (extension) {
					measuredNode._extension = extension.name;
					extension.measure(measuredNode as ExtensionNode, {
						documentDefinition: this.pdfDocument.extensionDocument ?? {},
						virtualFileSystem: this.pdfDocument.virtualfs,
						getStyle: (property) => this.styleStack.getProperty(property),
						measureBox: (dimensions) =>
							this.media.measureImageWithDimensions(measuredNode, dimensions),
					});
					return extendMargins(measuredNode);
				}
