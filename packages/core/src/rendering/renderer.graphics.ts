...
class RendererGraphics {
	...
	private readonly extensions: PdfCraftExtensions;
	private vectorState: {
		lineWidth?: number;
		dash?: string;
		lineJoin?: string;
	...
	constructor(pdfDocument: PDFDocument, extensions: PdfCraftExtensions = []) {
	...
	renderExtension(node: LayoutPdfNode): void {
		const extension = this.extensions.find((candidate) => candidate.name === node._extension);
		if (!extension?.render) {
			throw new Error(`No renderer registered for extension '${node._extension ?? "unknown"}'`);
		}
		const resolveFont = (family: string, bold: boolean, italic: boolean, fallback: string) => {
			const fontsFamily = family
				.split(",")
				.map((fontName) => fontName.trim().replace(/('|")/g, ""));
			const font = findFont(this.pdfDocument.fonts, fontsFamily, fallback);
		...
		if (node.link) {
			this.pdfDocument.link(node.x!, node.y!, node._width!, node._height!, node.link);
		}
		if (node.linkToPage) {
			const action = this.pdfDocument.ref({
				Type: "Action",
			...
			(action.end as () => void)();
			this.pdfDocument.annotate(node.x!, node.y!, node._width!, node._height!, {
				Subtype: "Link",
				Dest: [node.linkToPage - 1, "XYZ", null, null, null],
			} as PDFKit.Mixins.AnnotationOption);
		}
		if (node.linkToDestination) {
			this.pdfDocument.goTo(node.x!, node.y!, node._width!, node._height!, node.linkToDestination);
		}
	}
