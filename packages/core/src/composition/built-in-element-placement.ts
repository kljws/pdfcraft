...
export function createBuiltInElementPlacement(): ElementPlacementAdapter {
	return {
		placeImage: (writer, node, index) =>
			imageFeature.place(writer, node as LayoutImageNode, index),
		...
		placeAttachment: (writer, node, index) =>
			attachmentFeature.place(writer, node as LayoutAttachmentNode, index),
