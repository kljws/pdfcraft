...
import type { ExtensionNode, PdfCraftExtensions, TableRowGroupLayout } from "../types";
...
class DocPreprocessor {
	...
	preprocessNode(input: unknown, isSectionAllowed: boolean = false): PreprocessedPdfNode {
		...
		if (node.section) {
			...
			return this.preprocessText(node);
		} else if (this.extensions.some((extension) => extension.test(node as ExtensionNode))) {
			return node;
		} else {
