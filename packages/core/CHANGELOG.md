...
- Preprocessing now stamps node kinds through one `markNodeKind` helper that documents the in-place contract (node references, TOC items and repeated layout passes rely on object identity) instead of a cast in every feature, and measurement contexts accept a `PendingMeasureNode` so composite features measure their children without casting them back to preprocessed nodes.
...
- Changed benchmark commands to print results only to the console and removed generated Markdown/JSON report output.
- Began reorganizing `@pdfcraft/core` around independent vertical features by moving image, attachment and canvas behavior behind typed feature boundaries; moving text normalization, inline shaping, measurement, line construction, pagination and PDFKit rendering behind the text feature; moving ordered/unordered-list validation, counters, markers, sizing and layout behind the list feature; separating vertical-stack and columns preprocessing, measurement and layout into their own features; moving block/inline AcroForm validation, measurement, placement, one-time PDFKit initialization and rendering behind an AcroForm feature; moving table validation, grouped-body/header normalization, span-grid expansion, layout resolution, intrinsic sizing, pagination, borders and rounded fragments behind the table feature boundary; moving TOC registration, preprocessing, internal table generation, measurement and layout behind a TOC feature; moving section validation, child measurement, page-property inheritance and page creation behind a section feature; centralizing registered-extension detection, resources, measurement, placement, layout, rendering and page-break metadata behind a feature-neutral adapter; and isolating document-wide ID, page-reference and text-reference preprocessing as a shared service. Decorated stacks now own their validation and table-backed lowering through an injected preprocessing capability. Cross-feature work is supplied through narrow engine callbacks, and the public API remains unchanged.
- Separated feature-neutral node layout lifecycle rules from feature dispatch, including page breaks, margins, detached positioning, unbreakable transactions, vertical alignment and page-span height tracking.
- Replaced duplicated preprocessing, measurement and layout feature-condition chains with an ordered built-in feature registry and stage-specific processors, preserving each stage's detection precedence, extension fallback and internal span handling.
- Removed the pass-through `LayoutBuilderContent` and `DocMeasureContainers` classes; layout and measurement composition roots now supply feature contexts directly.
- Isolated PDFKit vector state, clipping, vertical alignment and watermark rendering from the shared graphics coordinator.
- Isolated cloned-fragment replay, coordinate offsets, page-number updates and background-vector ordering from the base element-writer façade.
- Removed concrete feature imports from the base element writer by introducing a composed placement adapter for image, canvas, extension, attachment and AcroForm page items.
- Removed concrete feature imports from printer resources and PDFKit rendering façades through dedicated resource/rendering composition roots; moved shared measured-watermark output and `pageBreakBefore` contracts to feature-neutral type ownership.
- Moved the concrete built-in matcher registry from engine to composition so engine, document, layout, rendering, resources, fonts and styles no longer select features directly.
- Moved shorthand normalization, ordered handler construction, concrete feature preprocessing contexts, extension fallback and TOC registration out of `DocPreprocessor` into preprocessing composition; `DocPreprocessor` now retains only document/reference state and generic recursive entry points, with historical feature-specific preprocessing façades removed.
- Extracted vector-insertion ownership tracking into a shared layout service, removed its test-only compatibility re-export from `ElementWriter` and kept the table feature independent of the concrete writer.
- Removed unused clip-item writer methods and pass-through page-writer context/position wrappers while retaining the real image, canvas, extension, attachment and AcroForm placement ports with their page-fit behavior.
- Made composition-only host/result contracts and element-placement internals module-private, replaced exported result interfaces with inferred factory return types and inlined trivial single-use measurement handlers.
- Moved static/dynamic header and footer layout, per-page overrides, detached overflow checks and footer-height measurement behind a documentary repeatables feature with a reduced engine context.
- Moved static/dynamic page-background resolution, page-count dependency detection, detached layout and background-item accounting behind a documentary repeatables feature.
- Moved document/per-page watermark selection, watermark contracts and PDFKit rendering behind a documentary repeatables feature, with font-aware text measurement supplied through a reduced global capability.
- Moved `pageBreakBefore` metadata construction, neighboring-node navigation and callback evaluation behind a documentary repeatables feature; feature-neutral position resets now belong to the engine relayout boundary.
- Added an explicit built-in document-feature composition root and removed the transitional `LayoutBuilderRepeatables` class; `LayoutBuilder` now invokes composed background, header/footer and watermark features directly.
- Moved watermark normalization and automatic sizing into the repeatables feature, and extracted style inheritance plus font-backed text metrics into feature-neutral services so documentary features no longer depend on text or layout internals; removed the transitional layout style façade and colocated its tests with the service.
- Extracted font-family/style resolution, VFS loading, local-file validation, PDFKit embedding and caching into a typography service while preserving the existing `PDFDocument` font methods and cache fields as compatible façades.
- Moved built-in measurement context construction, shared image/text measurers, handler registration, style-scoped node lifecycle, margin extension, extension fallback and unknown-node errors into a composition root; `DocMeasure` now owns only shared measurement state and generic document, block and node entry points, with historical feature-specific measurement façades removed.
- Extracted bounded document relayout orchestration from `LayoutBuilder` into an engine pipeline, preserving dynamic page-count/background convergence, footer-margin retries, `pageBreakBefore` retries, cycle warnings and the ten-pass footer error.
- Moved built-in pipeline composition for `pageBreakBefore` and registered-extension node metadata out of `LayoutBuilder`, leaving bounded convergence feature-neutral.
- Moved document processor creation and concrete pass setup—preprocessing, measurement, page context, background listener, automatic page height, headers, footers and watermarks—into document composition while retaining `layoutDocument`/`tryLayoutDocument` compatibility façades.
- Extracted vertical-spacing pagination and snaking column/page transitions from `LayoutBuilder` into a feature-neutral engine service while preserving builder façades and subclass overrides.
- Moved primary/trailing layout order, initial-page decisions, built-in feature handler construction and direct stack/section/column calls out of `LayoutBuilder` into a layout composition root. Shared table/columns row layout now stays inside composition, and the historical `processVerticalContainer`, `processSection`, `processColumns` and `processRow` pass-through façades were removed while retaining `layoutDocument` and real recursive entry points.
- Moved complete node-layout dispatch, registered-extension fallback and unknown-node validation into the layout composition root, removing concrete feature dispatch knowledge from `LayoutBuilder`.
- Removed the canvas dependency from generic node decoration; layout composition now supplies canvas coordinate decoration/reset through feature-neutral lifecycle hooks.
...
- Replaced page-wide geometric scans used to close rounded table fragments with direct references to the vectors emitted by the current `TableProcessor`. Only the registered vectors for the relevant page fragment are now inspected or invalidated.
...
- Updated the Git installation example to pin PDFCraft to the `v0.7.2` release tag.
...
- Replaced the three duplicated Kolecto invoice product groups with distinct audit, sensor deployment and platform-maintenance services, each with coherent references, quantities, prices and descriptions.
...
[0.6.1]: https://github.com/kljws/pdfcraft/compare/v0.6.0...v0.6.1
