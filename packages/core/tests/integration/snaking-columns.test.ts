...
describe("Integration test: snaking columns", function () {
	...
	it("should position wide canvas content correctly within snaking columns", function () {
		// Test handling of content that is wider than a single column
		// Wide canvas should either fit or flow appropriately
		var dd = {
			content: [
				{
					columns: [
						{
							stack: [
								{ text: "Content before canvas", fontSize: 12 },
								{
									canvas: [{ type: "rect", x: 0, y: 0, w: 200, h: 50, color: "#3498db" }],
								},
								{ text: "Content after canvas", fontSize: 12 },
		...
		var pages = testHelper.renderPages("A4", dd);
		assert.ok(pages.length >= 1, "Document should render canvas content inside snaking columns");
